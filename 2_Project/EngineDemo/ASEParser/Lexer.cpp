...
Lexer::Lexer(LPSTR fileName, DWORD sizeOfBuffer)
{
   m_hFile = NULL;
   m_TotalFileSize = 0;
   m_NowProgress = 0;
...
CHAR Lexer::GetNextChar()
{
   ...
   if (m_BufferPointer == m_NowLoadedBuffer)
   {
      if (ReadFile(m_hFile, m_Char, m_SizeOfBuffer, &m_NowLoadedBuffer, NULL) != TRUE)
      {
         return 0;
      }

      if (!m_NowLoadedBuffer)
      {
         return 0;
      }
   ...
   else
   {
      ...
      return m_Char[m_BufferPointer - 1];
   }
}

BOOL Lexer::Open(LPSTR fileName, DWORD sizeOfBuffer)
{
   ...
   // 2) 기본적인 변수들을 설정한다.
   m_TotalFileSize = GetFileSize(m_hFile, NULL);
   m_NowProgress = 0;
   m_BufferPointer = 0;
   m_NowLoadedBuffer = 0;
...
ASELexer::ASELexer(LPSTR fileName, DWORD sizeOfBuffer) : Lexer(fileName, sizeOfBuffer)
{
}
...
// Lexical Analysis
LONG ASELexer::GetToken(LPSTR tokenString)
{
   // 1) 앞쪽 쓸데없는 문자찾기
   //    앞쪽에 공백이나 Tab이나 개행이나 캐리지리턴 같은 문자열이 있으면 무시
   ...
   // 4) '*'인가를 확인
   //    만약 '*'로 시작하는 것은 Reserved word이므로 검색
   else if (Char == '*')
   {
   ...
   else if (Char == '\0') // ase파일 cdcdcdcd 버퍼 오버플로 예외처리 
   {
      return TOKEND_END;
   }
   // 7) 지금까지 못찾았음 이건 허당, 다음을 기약
   else
   {
      // 첫글자는 char가 보관하고 있으니 이것을 일단 tokenStirng에 넣고 다음 글자를 읽어 들인다.
      tokenString[0] = Char;
      Char = GetNextChar();
      if (!Char)
      {
