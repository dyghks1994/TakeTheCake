...
public:
	...
	/// 초기화, 로드
	bool Initialize();
	bool LoadFile(LPSTR p_File);		// ASE파일을 읽어서, 자체 리스트에 넣는다.
...
public:
	...
	Mesh* m_OneMeshData;						// 메시 하나의 데이터가 들어갈 구조체
	vector<Mesh*> m_MeshData_v;				// 메시들
	...
	list<Animation*> m_Animation_l;		// 그 데이터의 리스트
   vector<Animation*> m_AnimationByName_v;		/// (임시)여러가지 애니를 가진 것
