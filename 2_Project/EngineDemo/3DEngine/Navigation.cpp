...
Vector3 Navigation::VectorToKeepInTriangle(Vector3 pos)
{
   ...
   Vector3 N = NormalVectorFromTriangle(m_pNowTriangle); // N
   N.Normalize();
...
Vector3 Navigation::PositionInTriangle(Vector3 pos, Triangle* triangle)
{
   ...
   Vector3 trngNor = NormalVectorFromTriangle(triangle); // 삼각형의 노말벡터
...
Vector3 Navigation::NormalVectorFromTriangle(Triangle* triangle)
{
   // 삼각형의 노말 계산하기 
   Vector3 vecAB = triangle->b - triangle->a;
   Vector3 vecAC = triangle->c - triangle->a;
   //Vector3 _normal = vecAB.Cross(vecAC);
