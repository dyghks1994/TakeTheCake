...
BlurFilter::BlurFilter()
	: m_pBlurOutputTexSRV(0), m_pBlurOutputTexUAV(0)
{
}

BlurFilter::~BlurFilter()
{
	ReleaseCOM(m_pBlurOutputTexSRV);
	ReleaseCOM(m_pBlurOutputTexUAV);
}

ID3D11ShaderResourceView* BlurFilter::GetBlurredOutput()
{
	return m_pBlurOutputTexSRV;
}
...
void BlurFilter::SetGaussianWeights(float sigma)
{
	...
	StaticEffect::BlurFX->SetWeights(weights);
}
...
void BlurFilter::Init(ID3D11Device* device, UINT width, UINT height, DXGI_FORMAT format)
{
	...
	HR(device->CreateShaderResourceView(blurredTex, &srvDesc, &m_pBlurOutputTexSRV));

	D3D11_UNORDERED_ACCESS_VIEW_DESC uavDesc;
	uavDesc.Format = format;
	uavDesc.ViewDimension = D3D11_UAV_DIMENSION_TEXTURE2D;
	uavDesc.Texture2D.MipSlice = 0;
	HR(device->CreateUnorderedAccessView(blurredTex, &uavDesc, &m_pBlurOutputTexUAV));
	...
	int blurCount)
{
	...
	for (int i = 0; i < blurCount; ++i)
	{
		...
		StaticEffect::BlurFX->HorzBlurTech->GetDesc(&techDesc);
		for (UINT p = 0; p < techDesc.Passes; ++p)
		{
			StaticEffect::BlurFX->SetInputMap(inputSRV);
			StaticEffect::BlurFX->SetOutputMap(m_pBlurOutputTexUAV);
			StaticEffect::BlurFX->HorzBlurTech->GetPassByIndex(p)->Apply(0, dc);
		...
		// VERTICAL blur pass.
		StaticEffect::BlurFX->VertBlurTech->GetDesc(&techDesc);
		for (UINT p = 0; p < techDesc.Passes; ++p)
		{
			StaticEffect::BlurFX->SetInputMap(m_pBlurOutputTexSRV);
			StaticEffect::BlurFX->SetOutputMap(inputUAV);
			StaticEffect::BlurFX->VertBlurTech->GetPassByIndex(p)->Apply(0, dc);
