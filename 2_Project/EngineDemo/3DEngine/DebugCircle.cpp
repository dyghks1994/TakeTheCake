...
DebugCircle::DebugCircle(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, Matrix world, float radius)
   :m_pDevice(pDevice), m_pDeviceContext(pDeviceContext), m_World(world), m_Radius(radius), m_IsRendering(true)
{
   m_IsCollided = false;
}
...
void DebugCircle::BuildGeometryBuffer()
{
   ...
   HR(m_pDevice->CreateBuffer(&vbdRed, &vinitDataRed, &m_pVBRed));
   ...
   HR(m_pDevice->CreateBuffer(&vbdGreen, &vinitDataGreen, &m_pVBGreen));
   ...
   HR(m_pDevice->CreateBuffer(&ibdGreen, &iinitDataGreen, &m_pIB));
}
